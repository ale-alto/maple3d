...
- [x] Loot is per-player the CLASSIC way (revised 2026-07-14 from playtest): everyone sees the drops (foreign ones dimmed), only the killer can pick them up — test: `tests/e2e/multiplayer.spec.js::drops are visible to all but owner-protected`
- [x] Party members' star throws are visible (cosmetic replicas; server owns damage) — test: `tests/e2e/multiplayer.spec.js::remote throws are visible` *(added 2026-07-14 from playtest)*
...
- Playtest fixes 2026-07-14: name tags moved BELOW the character (classic MS placement); remote throws relayed as cosmetic stars; loot changed from invisible-to-others to visible-but-owner-protected (dimmed at 45% opacity for non-owners); pickups broadcast via server dropIds so drops vanish everywhere; remote level-ups detected from the presence stream (no protocol addition) and flashed over the character.
