...
**M12 + M13 DONE. M14 items IMPLEMENTED 2026-07-20** (suite 93/93, live-verified) — gate: user playtest. After M14 closes, next parity step (M15): Assassin@30 (Claw Mastery/Booster/Haste/Drain/Critical Throw — fetch bbb tables first), cap raise on the real curve; also open: mob-stat/defense-formula pass. M14 was: real throwing-star types with WA (Subi etc.), claw WA values replacing BASE_WA 30 interim, real potion table, mob stat pass (contact-damage formula vs the disorder wdef half).
